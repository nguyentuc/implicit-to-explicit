...
class ITE:
    ...
    def create_model(hyper_params):
        ...
        with tf.device("/gpu:0"):
            ...
            # --------------------------------- explicit part ------------------------------------
            im2ex_weights = {
                "w1": tf.Variable(tf.random_normal([2 * num_factor, 4 * num_factor]) * tf.sqrt(1 / (2 * num_factor)),
                                  name="im2ex_weight1"),
                "w2": tf.Variable(tf.random_normal([4 * num_factor, 2 * num_factor]) * tf.sqrt(1 / num_factor),
                                  name="im2ex_weight2"),
                "w3": tf.Variable(tf.random_normal([2 * num_factor, num_factor]) * tf.sqrt(2 / num_factor),
                                  name="im2ex_weight3"),
                "h": tf.Variable(tf.random_uniform([num_factor, 1], minval=-1, maxval=1), name="explicit_h")
            }
            im2ex_biases = {
                "b1": tf.Variable(tf.random_normal([4 * num_factor]), name="im2ex_bias1"),
                "b2": tf.Variable(tf.random_normal([2 * num_factor]), name="im2ex_bias2"),
                "b3": tf.Variable(tf.random_normal([num_factor]), name="im2ex_bias3")
            }
            # 1 x 4*num_factor
            ex_phi_1 = tf.nn.leaky_relu(tf.add(tf.matmul(im_phi, im2ex_weights["w1"]), im2ex_biases["b1"]),
                                        name="ex_phi_1")
            ex_phi_2 = tf.nn.leaky_relu(tf.add(tf.matmul(ex_phi_1, im2ex_weights["w2"]), im2ex_biases["b2"]),
                                        name="ex_phi_2")
            ex_phi_3 = tf.nn.leaky_relu(tf.add(tf.matmul(ex_phi_2, im2ex_weights["w3"]), im2ex_biases["b3"]),
                                        name="ex_phi_3")
            ex_bias = tf.Variable(0.0, name="ex_bias")
            train_ex_prediction = tf.squeeze(tf.add(tf.matmul(ex_phi_3, im2ex_weights["h"]), ex_bias),
