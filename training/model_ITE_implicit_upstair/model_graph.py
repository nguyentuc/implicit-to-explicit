...
class ITE:
    ...
    def get_embedding_weight(num_user, num_item, num_factor):
        ...
        num_factor_gmf = 2 * num_factor
        num_factor_mlp = 4 * num_factor
    ...
    def create_model(hyper_params):
        ...
        with tf.device("/gpu:0"):
            ...
            mlp_weights = {
                "w1": tf.Variable(tf.random_normal([8 * num_factor, 4 * num_factor]) * tf.sqrt(1 / (2 * num_factor)),
                                  name="mlp_weight1"),
                "w2": tf.Variable(tf.random_normal([4 * num_factor, 2 * num_factor]) * tf.sqrt(1 / num_factor),
                                  name="mlp_weight2")
            }
            mlp_biases = {
                "b1": tf.Variable(tf.random_normal([4 * num_factor]), name="mlp_bias1"),
                "b2": tf.Variable(tf.random_normal([2 * num_factor]), name="mlp_bias2")
            }
            ...
            im_weights = {
                "w1": tf.Variable(tf.random_normal([4 * num_factor, 2 * num_factor]) * tf.sqrt(1 / num_factor),
                                  name="im_weight1"),
                "h": tf.Variable(tf.random_uniform([2 * num_factor, 1], minval=-1, maxval=1), name="implicit_h")
            }
            im_biases = {
                "b1": tf.Variable(tf.random_normal([2 * num_factor]), name="im_bias1")
            }
            ...
            # 1 x num_factor
            ex_phi = tf.nn.leaky_relu(tf.add(tf.matmul(im_phi_2, im2ex_weights["w1"]), im2ex_biases["b1"]),
                                      name="ex_phi")
